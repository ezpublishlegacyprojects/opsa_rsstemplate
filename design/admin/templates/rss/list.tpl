{* Export window. *}
<form name="rssexportslist" method="post" action={'rsstemplate/list'|ezurl}>
...
<tr class="{$RSSExports.sequence}">
    ...
    {* Name. *}
    <td>{section show=$RSSExports.item.active|eq( 1 )} <a href={concat( 'rsstemplate/feed/', $RSSExports.item.access_url )|ezurl}>{$RSSExports.item.title|wash}</a>{section-else}{$RSSExports.item.title|wash}{/section}</td>
    ...
    {* Edit. *}
    <td><a href={concat( 'rsstemplate/edit_export/', $RSSExports.item.id )|ezurl}><img class="button" src={'edit.gif'|ezimage} width="16" height="16" alt="{'Edit'|i18n( 'design/admin/rss/list' )}" title="{'Edit the <%name> RSS export.'|i18n('design/admin/rss/list',, hash( '%name', $RSSExports.item.title) )|wash}" /></a></td>
